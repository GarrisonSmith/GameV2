...
{
	public class SpriteSheetAnimationSet : Animation
	{	
		protected byte activeFrameRow;
		...
		/// <summary>
		/// Gets or sets a value indicating if the <c>SpriteSheetAnimationSet</c> is paused.
		/// Initialized to false when all <c>SpriteSheetAnimationSet</c> objects are created.
		/// </summary>
		...
		/// <summary>
		/// Gets the column of the currently drawn frame in the spritesheet of the <c>SpriteSheetAnimationSet</c>.
		/// </summary>
		public byte ActiveFrameRow { get => this.activeFrameRow; protected set => this.activeFrameRow = value; }
		/// <summary>
		/// Gets the minimum duration a frame will persist for in this <c>SpriteSheetAnimationSet</c>.
		/// </summary>
		public int MinMillisecondDuration { get => this.minMillisecondDuration; }
		/// <summary>
		/// Gets the maximum duration a frame will randomly be extended for beyond the minimum duration in this <c>SpriteSheetAnimationSet</c>.
		/// </summary>
		public int MaxDurationMillisecondExtension { get => this.maxDurationMillisecondExtension; }
		/// <summary>
		/// Gets the current sheet box used by the <c>SpriteSheetAnimationSet</c>.
		/// </summary>
		public Rectangle CurrentSheetBox { get => this.currentSheetBox; protected set => this.currentSheetBox = value; }
		/// <summary>
		/// Gets the currently off set position of this <c>SpriteSheetAnimationSet</c>.
		/// </summary>
		public Vector2 CurrentOffSetPosition { get => this.currentOffSetPosition; protected set => this.currentOffSetPosition = value; }
		/// <summary>
		/// Gets the array of frames that define the <c>SpriteSheetAnimationSet</c>.
		/// </summary>
		public SpriteSheetFrame[] Frames { get => this.frames; }

		/// <summary>
		/// Creates a new <c>SpriteSheetAnimationSet</c> with the provided parameters.
		/// </summary>
		/// <param name="sheetBox">The sheet box. The most top left frame in the <c>SpriteSheetAnimationSet</c>.</param>
		/// <param name="spriteSheet">The sprite sheet.</param>
		/// <param name="position">The position.</param>
		/// <param name="animationElement">The animation element.</param>
		public SpriteSheetAnimationSet(Rectangle sheetBox, Texture2D spriteSheet, PositionRef position, XmlElement animationElement) : base(sheetBox, spriteSheet, position)
		{
			foreach (XmlElement foo in animationElement)
			{
				...
				else if (foo.Name.Equals("activeFrameRow"))
				{
					this.ActiveFrameRow = byte.Parse(foo.InnerText);
				}
			...
			this.CurrentSheetBox = new Rectangle(this.SheetBox.X + (this.ActiveFrameColumn * this.SheetBox.Width), this.SheetBox.Y + (this.ActiveFrameRow * this.SheetBox.Height), this.SheetBox.Width, this.SheetBox.Height);
		...
		/// <summary>
		/// Draws the <c>SpriteSheetAnimationSet</c> using the specified <c>GameTime</c>.
		/// </summary>
